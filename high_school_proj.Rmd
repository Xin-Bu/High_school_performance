...
### Data Visualization
#### Create a new table to summarize test preparation by race

```{r, message=FALSE}
new_test_prep <- hs %>%
  group_by(Eth,Test_prep)%>%
  summarise(N=n())%>%
  mutate(freq = N/sum(N),
         pct = round(freq*100),0)

new_test_prep%>% group_by(Test_prep)%>%
  summarize(total=sum(pct))

```

#### Visualize the percentage of test prepartion by race

```{r}
ggplot(new_test_prep, aes(x=Eth, y=pct, fill=Test_prep))+
  geom_col() +
  labs(x = "Race", y = "Percent", title= "Test Preparation by Race")+
...
#### Reorder the categorical variable Par_edu and then visualize the relationship between math and reading by parental education level and lunch type
...
ggplot(hs, aes(x = Eth, fill = Par_edu)) + 
  geom_bar(alpha = 1, position = "dodge2")+
  labs(x="Race/ethnicity", y="Count", title = "Parental Education by Race/Ethnicity")+
